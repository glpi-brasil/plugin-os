...
$SelDataInternal = "SELECT internal_time_to_own,date_format(internal_time_to_own, '%d/%m/%Y %H:%i') AS DataInternalown FROM glpi_tickets WHERE id = '".$_GET['id']."'";
$ResDataInternal = $DB->query($SelDataInternal);
$DataInternal = $DB->fetch_assoc($ResDataInternal);
$OsDataInternal = $DataInternal['DataInternalown'];

$SelDataInternalFinal = "SELECT internal_time_to_resolve,date_format(internal_time_to_resolve, '%d/%m/%Y %H:%i') AS DataInternalFim FROM glpi_tickets WHERE id = '".$_GET['id']."'";
$ResDataInternalFinal = $DB->query($SelDataInternalFinal);
$DataInternalFinal = $DB->fetch_assoc($ResDataInternalFinal);
$OsDataInternalEntrega = $DataInternalFinal['DataInternalFim'];

$SelTicketFollowups = "SELECT * FROM `glpi_itilfollowups` WHERE items_id = '".$_GET['id']."'";
$ResTicketFollowups = $DB->query($SelTicketFollowups);
